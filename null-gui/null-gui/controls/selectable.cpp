...
namespace null_gui {
	bool selectable(std::string text, bool selected) {
		...
		rect item_rect = rect(draw_pos, draw_pos + math::max(vec2(null_gui::gui_settings::items_size_full_window ? math::max(min.x, wnd->get_window_size_with_padding().x) : min.x, min.y), vec2(0.f, 0.f)));
