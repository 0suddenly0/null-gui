...
namespace null_gui {
	void slider_int(std::string text, int* value, int min, int max, std::string format, int step, int ctrl_step) {
		...
		int new_value = math::clamp(min + (max - min) * (null_input::mouse_pos().x - size.min.x) / size.size().x, (float)min, (float)max);
		...
		wnd->draw_list->draw_rect_filled(size_body.min, size_body.max, hovered || pressed ? gui_settings::button_bg_hovered : gui_settings::button_bg);
		...
		if (hovered) {
			...
			wnd->draw_list->draw_rect_filled(size_body.min, vec2(size_body.min.x + slider_size_hovered, size_body.max.y), color(gui_settings::main_color, 100));
		}

		float slider_size = ((float)(clamped - min) / (float)(max - min)) * size.size().x;
		wnd->draw_list->draw_rect_filled(size_body.min, vec2(size_body.min.x + slider_size, size_body.max.y), gui_settings::main_color);
	...
	void slider_float(std::string text, float* value, float min, float max, std::string format, float step, float ctrl_step) {
		...
		vec2 right_spacing(gui_settings::spacing_checkbox_size ? gui_settings::checkbox_size + gui_settings::text_spacing : wnd->get_scroll_offset(), 0.f);
		rect size(draw_pos + left_spacing, vec2(draw_pos.x + wnd->size.x - gui_settings::window_padding.x - (right_spacing.x + gui_settings::window_padding.x), draw_pos.y + text_size.y + gui_settings::slider_size + gui_settings::text_spacing));
		rect size_body(vec2(size.min.x, size.max.y - gui_settings::slider_size), size.max);
		float new_value = math::clamp(min + (max - min) * (null_input::mouse_pos().x - size.min.x) / size.size().x, min, max);
