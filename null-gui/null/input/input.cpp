...
namespace null {
	namespace input {
		...
		void input_key::update_duration() {
			down_duration = down() ? (down_duration < 0.0f ? 0.0f : down_duration + gui::detail::delta_time) : -1.0f;
		}
		...
		bool process_keybd_message(UINT msg, WPARAM w_param, LPARAM l_param) {
			...
			key->state_down = state;
			if(state) {
				gui::detail::text_input_info::win_poc((key_id)id);
				last_press_key = id;
			}
		...
		LRESULT wnd_proc(UINT msg, WPARAM w_param, LPARAM l_param) {
			...
			case WM_CHAR:
				...
				gui::detail::text_input_info::add_char();
				return true;
			case WM_MBUTTONDBLCLK:
			case WM_RBUTTONDBLCLK:
			case WM_LBUTTONDBLCLK:
		...
		void update_keys_state() {
			...
			for(input_key& key : key_list) {
				...
				if(key.state_clicked) {
					if((float)(gui::detail::real_time - key.clicked_time) < settings::double_click_time) {
						vec2 delta = mouse_pos - mouse_click_pos;
						if(delta.length() < pow(settings::double_click_max_dist, 2)) {
						...
						key.clicked_time = gui::detail::real_time;
					}
		...
		void write_clipboard(std::string value) {
			if(OpenClipboard(gui::detail::hwnd)) {
				EmptyClipboard();
				HGLOBAL data = GlobalAlloc(GMEM_DDESHARE, value.length() + 1);
				char* buffer = (char*)GlobalLock(data);
				strcpy(buffer, LPCSTR(value.c_str()));
				GlobalUnlock(data);
		...
		std::string read_clipboard() {
			...
			if(OpenClipboard(gui::detail::hwnd)) {
				...
				if(data) result = (char*)GlobalLock(data);
				GlobalUnlock(data);
				CloseClipboard();
			}
			return result;
